...
module.exports = function(config, eventsCB) {
  this.config = config;
  this.calendars = this.config.calendars;
  this.calendarEvents = {};
  this.eventsCB= eventsCB;
  
  if (this.config.locale) {
    moment.locale(this.config.locale);
  }

  this.startScanCalendars= function() {
  ...
  this.scanCalendar= function(calendar, cb) {
  ...
  this.parser= function(calendar, iCalData=null, error=null) {
    if (error) {
      console.log(`[CALEXT2]E1 calendar:${calendar.name} >> ${error.message}`)
      return
    }
    if (!iCalData) {
    ...
    try {
      ...
      console.log(`[CALEXT2]E2 calendar:${calendar.name} >> ${e.message}`)
      return
    }
    ...
    try {
      ...
      console.log(`[CALEXT2]E3 calendar:${calendar.name} >> ${e.message}`)
      return
    }
  ...
  this.mergeEvents= function(eventPool, calendarId) {
